...
namespace framework::network {
	class service : public design_pattern::singleton<service> {
	private:
		friend class design_pattern::singleton<service>;
		friend class server;
		enum class type : unsigned char {
			close, destory_session, task, destory_group
		};

		inline explicit service(void) noexcept {
		...
		inline explicit service(service const&) noexcept = delete;
		inline explicit service(service&&) noexcept = delete;
		inline auto operator=(service const&) noexcept -> service & = delete;
		inline auto operator=(service&&) noexcept -> service & = delete;
		inline ~service(void) noexcept {
	...
	public:
		inline void start(void) noexcept {
			...
			for (unsigned short index = 0; index < 16; ++index)
				_worker_thread.emplace_back(&service::worker, 0, this);

			_scheduler.initialize();
			_scheduler._thread.begin(&service::schedule, 0, this);
		}
